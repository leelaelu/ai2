...
# Google Drive 파일 ID
file_id = '1udWysKk_SXJah6cwMSff3ZZDMFFiHlog'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/Lz41yBZ/images.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://youtu.be/rQbYJZ_r2iA?feature=shared", start_time=0)
